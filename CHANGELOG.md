...
Published as `@engram-ai-memory/core`, `@engram-ai-memory/mcp` and `@engram-ai-memory/cli` 0.7.0, `@engram-ai-memory/vis` 0.5.0, and the Claude Desktop extension bundle 0.7.0.
...
- **`@engram-ai-memory/server`**, **`@engram-ai-memory/cli`** — An `ENGRAM_SYNC_ENCRYPTION_KEY` that is set but empty is now refused: the server aborts startup, and `engram cloud sync` and `engram cloud status` fail with the same message instead of running. `''` is falsy, so `if (!encryptionKey)` read it as "no encryption configured" and the whole local store was pushed to Postgres in the clear while the config that set the variable still said encryption was on. Core refuses to push plaintext at a database that already holds ciphertext, but a fresh sync target has no history to check against, and that is the case that ends with an unencrypted copy of every memory on a remote database. Same rule as `ENGRAM_API_KEY` above: unset means "no encryption wanted", empty means "wanted, value lost".

- **`@engram-ai-memory/server`** — A blank `HOST` binds loopback instead of every interface. `process.env['HOST'] ?? '127.0.0.1'` keeps the empty string — `??` falls back only when a variable is *absent* — and `listen({ host: '' })` listens on `0.0.0.0`, so a host templating an untouched optional field published on the whole network an API whose documented default is loopback and whose default is no authentication. Blank now means unset. **A deployment that was reachable off-box because `HOST` was set to nothing must now set `HOST=0.0.0.0` explicitly.** It is the same defect as the Ollama proxy's bind host below, in the process that holds the memories.

- **`@engram-ai-memory/server`**, **`@engram-ai-memory/adapter-ollama`** — A malformed value in a variable that bounds something now stops the process at startup rather than being silently ignored. `parseInt` answers `NaN` and `NaN` survives `??` — it is not nullish — so each of these disabled the thing it was meant to configure while everything still reported it as set. `ENGRAM_SYNC_INTERVAL=5m` reached `setInterval(NaN)`, a timer firing about every millisecond against the user's own Postgres. `ENGRAM_MAX_BODY_BYTES=10mb` did not raise the body cap on an unauthenticated proxy, it removed it — `size > NaN` is false for every size — while the startup banner went on printing a limit. `PORT` and `OLLAMA_PROXY_PORT` reached `listen()` as a request for an arbitrary free port, so the process came up somewhere nothing was configured to talk to. `ENGRAM_SYNC_MODE` was cast with `as`, so an unrecognised mode passed intact to `SyncEngine`, whose `mode !== 'auto'` check then turned the scheduler off: a typo silently disabled sync. Values parse with `Number` rather than `parseInt`, so `10mb` is an error rather than `10`. (`ENGRAM_NAMESPACE_MODE` already refused an unrecognised value and still does; it reads through the same helper now, so a whitespace-only value counts as unset instead of aborting.)

- **`@engram-ai-memory/server`** — Every `GET` route with a query schema answers `400` for an unknown query parameter: `/api/memory`, `/api/tags/:tag`, `/api/graph/:id`, `/api/graph/edges`, `/api/contradictions`, `/api/reflections`, `/api/webhooks` and `/api/recall/stream`. Fastify's ajv runs with `removeAdditional`, so an unknown key was stripped and the request answered `200` — `?limt=5` on `/api/memory` was read as no page size at all and answered with the default 50, and `?minStrenght=0.9` on `/api/graph/edges` was answered with every edge in the store as though it had been filtered. Only `/api/graph/edges` actually lied about it, declaring `additionalProperties: false` without the hook that enforces it; the other seven declared nothing and were honestly permissive, but the failure a caller sees is the same wrong answer with a `200` on it. Every in-repo client sends only known parameters.

- **`@engram-ai-memory/core`**, **`@engram-ai-memory/server`** — `stats().graphEdges` and `GET /api/stats`'s `graphEdges` count connections, not directions. A bidirectional link is stored as a mirrored pair of adjacency entries and summing the adjacency lists counted both halves, so on a store where most links are bidirectional the number was exactly double: on the 651-node graph measured elsewhere in this section, `graphEdges` read 16,984 beside `GET /api/graph/edges`'s own `stored` count of 8,492 — two numbers for one noun. It reads 8,492 now and the two agree. **The value roughly halves on a bidirectional-heavy graph**; `engram stats`' `Edges:` line is where it is most visible.
...
- **`@engram-ai-memory/adapter-ollama`** — The proxy binds `127.0.0.1` instead of every interface. It listened wide while Ollama itself defaults to loopback, so any peer on the network could drive the user's GPU through an unauthenticated proxy. **Reaching the proxy from another machine now requires `ENGRAM_PROXY_HOST`**; the startup banner says which mode is active. A blank `ENGRAM_PROXY_HOST` counts as unset for the same reason: `'' ?? '127.0.0.1'` keeps the empty string and `listen(port, '')` binds every interface, so narrowing the default alone would have left a compose file that templates an untouched optional field still handing the LAN an unauthenticated endpoint onto the GPU.
...
- **`@engram-ai-memory/cli`** — `ENGRAM_DEVICE_NAME` is removed. `engram start` and `engram update` exported it into the server's environment, where nothing has ever read it: the server's identity is the per-install device id minted in core, and `engram cloud status` reads the name straight from the CLI's own config file. The `deviceName` config key is unchanged and still labels this device in `cloud status` and `cloud devices`. A setting that only looks like it does something is worse than no setting.
...
- **`@engram-ai-memory/core`**, **`@engram-ai-memory/server`**, **`@engram-ai-memory/adapter-ollama`** — A malformed tuning knob — `ENGRAM_WEBHOOK_MAX_CONCURRENCY`, `ENGRAM_BATCH_CONCURRENCY`, `ENGRAM_DECAY_INTERVAL`, `ENGRAM_DECAY_THRESHOLD`, `ENGRAM_MAX_TOKENS`, `ENGRAM_UPSTREAM_TIMEOUT_MS` — now names itself on stderr and uses the documented default. `ENGRAM_WEBHOOK_MAX_CONCURRENCY=many` parsed to `NaN`, and `inFlight >= NaN` is false for every value of `inFlight`, so the ceiling that stops one batch store from stacking hundreds of detached deliveries vanished entirely — at the exact moment someone had tried to configure it. `ENGRAM_BATCH_CONCURRENCY` fell back through `|| 16`, which also turned a deliberate `0` into 16 and clamped a negative to 1. The decay pair already fell back correctly but said nothing about it. None of these abort startup, unlike the bounds above — a wrong batch size costs throughput, not safety — but a typo is no longer indistinguishable from never having set the variable.
...
- **`@engram-ai-memory/cli`**, **`@engram-ai-memory/mcp`** — The two package READMEs npm renders were describing an older program. The CLI README documented four of the six `engram cloud` subcommands, omitted reflections entirely, and said nothing about exit codes — which is the whole interface for anyone scripting it. The MCP README gave a database path default that is no longer the default and left out the sync variables added since.

- **Documentation** — Three claims elsewhere were wrong rather than stale. `docs/INTEGRATIONS.md` said the CLI "works directly with the database — no running server needed"; it speaks HTTP to the REST API, and every data command fails without a server. The same page told readers to set `ENGRAM_DB_PATH` to reach that database, which does nothing at all: the CLI never reads the ambient variable, and the path it hands to a server it starts comes from its own config file. `docs/CONFIGURATION.md` put the MCP block in `~/.claude/settings.json`, where `engram doctor` warns about it, rather than `~/.claude.json`. `docs/CLOUD-SYNC.md` also contradicted itself about copied databases — section 4 still described the pre-fix behaviour that section 10 corrects — and the Smithery badge and links pointed at a server path that no longer resolves.
...
- **`@engram-ai-memory/core`**, **`@engram-ai-memory/cli`** — Three more variables where a blank value beat the default, found by sweeping every `??` read of `process.env`. A blank `ENGRAM_EMBEDDING_MODEL` was handed to the transformers pipeline as a model name and stamped onto every row as the model that produced its vector. A blank `ENGRAM_INDEX_PATH` resolved to `''` and silently disabled index persistence, so the vector index was rebuilt from the store on every start. A blank `ENGRAM_HOME` made `config.json`, `server.pid` and the log path resolve relative to whatever directory `engram` was run from, so the CLI found a different install per shell. Blank means unset in all three.
...
[Unreleased]: https://github.com/ayvazyan10/engram/compare/v0.7.0...HEAD
[0.7.0]: https://github.com/ayvazyan10/engram/releases/tag/v0.7.0
